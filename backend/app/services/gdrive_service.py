...
Auto-creates folder structure:
  My Drive / CAR-AGENTS Contracts / {Customer Name} / {contract.pdf}
...
async def upload_approved_contract_to_drive(
    ...
    My Drive / CAR-AGENTS Contracts / {Customer Name} / {contract_filename}
    ...
    try:
        ...
        # Step 2: Get/create customer subfolder
        customer_folder_id = _find_or_create_folder(clean_customer, parent_id=root_id)
        logger.info(f"Customer folder ID for '{clean_customer}': {customer_folder_id}")

        # Step 3: Upload PDF using multipart upload
        ...
        return {
            ...
            "drive_url": drive_url,
            "folder_path": f"My Drive / {ROOT_FOLDER_NAME} / {clean_customer}",
            "message": f"Contract saved to Google Drive under 'CAR-AGENTS Contracts / {clean_customer}'."
        }
